...
- **`OcrProfile.prompt_for`** ([`paperplane/ollama_ocr.py:36-56`](../paperplane/ollama_ocr.py))
  dispatches a different Markdown prompt per detected region label and model family
  (e.g. GLM-OCR gets `"Table Recognition:"` for table-like labels; DeepSeek-OCR gets
  `"<|grounding|>Convert the document to markdown."` for the same case). The prompt text lives
  in `paperplane/prompts/`, while Python handles family and region dispatch.
