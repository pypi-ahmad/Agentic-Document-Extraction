...
Set `OPENAI_API_KEY` and optional `OPENAI_BASE_URL` as Windows user environment variables,
then open a new terminal. Alternatively use ignored `.env` or Streamlit secrets. Native
PDFs and supported Office/OpenDocument files do not require the key.
