...
async def test_extract_node_exhausts_retries(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr("app.services.llm.registry.get_llm_provider", lambda pid: AlwaysRetryableLLM())
    monkeypatch.setattr("app.services.extraction.graph._RETRY_BASE_DELAY", 0.0)
...
@pytest.mark.asyncio
async def test_full_pipeline_confidence_triggers_review(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
...
async def test_extraction_response_includes_confidence(client):
    ...
    async with _test_session_maker() as db:
        doc = Document(
            ...
            file_path="/tmp/t.pdf", file_type="pdf", file_size=100,
        )
        ...
        await db.flush()
        ext = Extraction(
            document_id=doc.id, schema_id=schema.id,
            status="completed",
            result={"vendor": "Acme"},
