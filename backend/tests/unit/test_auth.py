...
async def test_parse_jobs_require_matching_api_key_when_set(monkeypatch) -> None:
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        missing = await client.get("/v2/parse/jobs")
        wrong = await client.get("/v2/parse/jobs", headers={"X-API-Key": "nope"})
        correct = await client.get("/v2/parse/jobs", headers={"X-API-Key": "secret"})
