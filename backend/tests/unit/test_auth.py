...
async def test_parse_jobs_are_open_when_api_key_is_unset(monkeypatch) -> None:
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        response = await client.get("/api/parse-jobs")
...
async def test_parse_jobs_require_matching_api_key_when_set(monkeypatch) -> None:
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        missing = await client.get("/api/parse-jobs")
        wrong = await client.get("/api/parse-jobs", headers={"X-API-Key": "nope"})
        correct = await client.get("/api/parse-jobs", headers={"X-API-Key": "secret"})
