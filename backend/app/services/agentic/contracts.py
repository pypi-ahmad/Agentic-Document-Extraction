"""Public DPT-style Parse/Extract contracts and deterministic grounding assembly."""
...
class StructureNode(BaseModel):
    ...
    def validate_node_shape(self) -> StructureNode:
        ...
        elif self.box is None or not self.ranges:
            raise ValueError("block nodes require a normalized box and at least one range")
...
class ParseResponse(BaseModel):
    ...
    def validate_grounded_document(self) -> ParseResponse:
        ...
        for page_number, page in enumerate(pages, start=1):
            if page.type != "page" or page.id != f"page-{page_number}" or page.page != page_number:
                raise ValueError("pages must be contiguous and ordered")
            for block in page.children:
                self._validate_block(block, page_number, expected_ids, parent_type=None)
        return self
...
class ExtractionResponse(BaseModel):
    ...
    @model_validator(mode="after")
    def require_mirrored_grounding_metadata(self) -> ExtractionResponse:
        self._validate_metadata_node(self.extraction, self.extraction_metadata)
        return self

    @classmethod
    def _validate_metadata_node(cls, value: Any, metadata: Any) -> None:
        if isinstance(value, dict):
            if not isinstance(metadata, dict) or set(metadata) != set(value):
                raise ValueError("extraction metadata must mirror object fields")
            for key, child in value.items():
                cls._validate_metadata_node(child, metadata[key])
            return
        if isinstance(value, list):
            if not isinstance(metadata, list) or len(metadata) != len(value):
                raise ValueError("extraction metadata must mirror array items")
            for child, child_metadata in zip(value, metadata, strict=True):
                cls._validate_metadata_node(child, child_metadata)
            return
        if not isinstance(metadata, dict) or set(metadata) != {"value", "ranges"}:
            raise ValueError("extraction metadata leaves require value and ranges")
        if metadata["value"] != value:
            raise ValueError("extraction metadata leaf value must mirror extraction value")
        if not isinstance(metadata["ranges"], list):
            raise ValueError("extraction metadata ranges must be a list")
        for item in metadata["ranges"]:
            CodepointRange.model_validate(item)
...
def assemble_parse_response(
    *,
    ...
    if [page.page_number for page in pages] != list(range(1, len(pages) + 1)):
        raise ValueError("input pages must be contiguous and ordered")
    ...
    for page_index, page in enumerate(pages):
        if page_index:
            append("\n\n")
        append(f"<!-- page_number={page.page_number} -->")
