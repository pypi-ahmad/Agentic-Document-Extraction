...
powershell.exe -NoProfile -Command "$root = [IO.Path]::GetFullPath($env:PAPERPLANE_ROOT); $shell = Get-CimInstance Win32_Process -Filter ('ProcessId = ' + $PID); $currentLauncher = $shell.ParentProcessId; $stopped = $false; $processes = Get-CimInstance Win32_Process; $oldLaunchers = @($processes | Where-Object { $_.ProcessId -ne $currentLauncher -and $_.Name -eq 'cmd.exe' -and $_.CommandLine -like ('*' + $root + '\Paperplane.cmd*') }); foreach ($launcher in $oldLaunchers) { taskkill.exe /PID $launcher.ProcessId /T /F *> $null; $stopped = $true }; $connection = Get-NetTCPConnection -LocalPort 8551 -State Listen -ErrorAction SilentlyContinue | Select-Object -First 1; if ($connection) { $owner = Get-CimInstance Win32_Process -Filter ('ProcessId = ' + $connection.OwningProcess); if ($owner.Name -eq 'python.exe' -and $owner.CommandLine -match '(?:paperplane[.]streamlit_runner|streamlit).*workspace_app[.]py') { $parent = Get-CimInstance Win32_Process -Filter ('ProcessId = ' + $owner.ParentProcessId); if ($parent.Name -eq 'python.exe' -and $parent.CreationDate -le $owner.CreationDate -and $parent.CommandLine -match ([regex]::Escape($root + '\.venv\Scripts\python.exe')) -and $parent.CommandLine -match '(?:paperplane[.]streamlit_runner|streamlit).*workspace_app[.]py') { $owner = $parent } }; if ($owner.CommandLine -match ([regex]::Escape($root + '\')) -and $owner.CommandLine -match '(?:paperplane[.]streamlit_runner|streamlit).*workspace_app[.]py') { $launcher = Get-CimInstance Win32_Process -Filter ('ProcessId = ' + $owner.ParentProcessId); if ($launcher.Name -eq 'cmd.exe' -and $launcher.ProcessId -ne $currentLauncher -and $launcher.CreationDate -le $owner.CreationDate -and $launcher.CommandLine -match '(?i)(?:^|[\\\s\x22])Paperplane[.]cmd(?:[\x22\s]|$)') { $owner = $launcher }; taskkill.exe /PID $owner.ProcessId /T /F *> $null; if ($LASTEXITCODE -ne 0) { exit 20 }; $stopped = $true } }; $deadline = (Get-Date).AddSeconds(5); do { $remaining = Get-NetTCPConnection -LocalPort 8551 -State Listen -ErrorAction SilentlyContinue | Select-Object -First 1; if (-not $remaining) { if ($stopped) { exit 10 }; exit 0 }; Start-Sleep -Milliseconds 200 } while ((Get-Date) -lt $deadline); exit 20"
